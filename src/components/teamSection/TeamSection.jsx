    ...
    <Box sx={{ py: { xs: 6, md: 10 } }}>
      <Container maxWidth="lg" disableGutters>
        ...
        <Box sx={{ textAlign: "center", mb: { xs: 5, md: 7 } }}>
          <Typography variant="h4" fontWeight={700} mb={1}>{t('Meet Our Team')}</Typography>
          <Typography color="text.secondary">{t('team_desc')}</Typography>
        </Box>
          ...
          {team.map((member) => (
            <Grid item size={{ xs: 12, sm: 6, md: 3 }} key={member.name}>
              <Box sx={{ textAlign: "center" }}>
                <Avatar src={member.image} alt={member.name} sx={{ width: 200, height: 200, mx: "auto", mb: 3 }} />
